...
echo "lugar_magico_id,fotografia,titulo,tiene,comentario<br>";
$contador = 1;
foreach($estados as $estado_id => $estado) {
    for($numero = 1; $numero <= 5; $numero++) {
        foreach ($categorias as $categoria_id => $categoria) {
            $tmp1 = str_pad($contador, 3, '0', STR_PAD_LEFT);
            $tmp2 = str_pad($categoria_id, 2, '0', STR_PAD_LEFT);
            echo "{$contador},{$tmp1}-{$tmp2}.jpg,Escribe un título para la fotografía,No,<=¿Lugar mágico {$contador} tiene una fotografía de '{$categoria}'? Si-No<br>";
        }
        $contador++;
    }
}
